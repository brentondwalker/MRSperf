#
# This version of the script processes a spark log as if the system were a (k,l) system
# as described in the Fidler-Jiang paper.  That is, a job with k tasks is considered complete
# once l of them complete.  In this model the remaining (k-l) tasks are left to finish.  That
    ...
    with gzip.open(args.file, 'r') as f:
        for line in f:
            ...
            if evt['Event'] == 'SparkListenerJobStart':
                ...
                events[job_id]['num_tasks_complete'] += 1
                if events[job_id]['num_tasks_complete'] == args.l:
                    events[job_id]['completion_time'] = task['finish_time']
                    total_jobs += 1
